# user_pgdpo_with_projection.py — Dollar PMP projection head (student φ estimator inside)
# -----------------------------------------------------------------------------
# Exposes:
#   - PP_NEEDS: list of keys the core must pass
#   - project_pmp(costates, states) -> π (B,d)
#
# Modes for rho:
#   PGDPO_RHO_MODE = student | regress | ratio
#     student (default): estimate φ schedule once per policy version by
#                        rolling out CURRENT_POLICY and computing
#                        λ_M, λ_V, and diagonal costates per step (student recipe).
#     regress:           JX ≈ a·X + b (per-batch) ⇒ rho = -(aX+b)/a = φ - X
...
def _rho_regress(JX, X):
    ...
    a = cov_xy / var_x
    # enforce concavity (a should be < 0 in MV precommitment)
    a = -torch.abs(a)
    b = y_mean - a * x_mean
    phi = (-b / a).view(1,1)          # (1,1), constant within the batch at fixed t
    rho = (phi - X)                    # (B,1)
    return rho.detach()
...
def _estimate_phi_schedule_student(policy) -> torch.Tensor:
    ...
    # λ_x = ∂J~/∂x_k  (vector over k), averaged across batch
    ...
    # ∂λ_x/∂x (Hessian diagonal approx) — stride to save compute
...
def _get_phi_for_tau(policy_ver: int, tau: torch.Tensor):
    """Return φ(tau) for each sample in batch using cached schedule (re-estimate on policy update)."""
    ...
    if need_refresh:
        # fetch current policy dynamically from base module
        from user_pgdpo_base import CURRENT_POLICY  # reimport to get latest ref
        _PHI_CACHE = _estimate_phi_schedule_student(CURRENT_POLICY).to(device=device)  # (m,)
        _PHI_CACHE_VER = policy_ver
        # reset refresh flag to avoid repetitive recompute
        os.environ["PGDPO_PHI_REFRESH"] = "0"

    dt = T / m
    # tau = T - t, index k ≈ floor((T - tau)/dt) = floor(t/dt)
...
def project_pmp(costates: dict, states: dict) -> torch.Tensor:
    ...
    if _RHO_MODE.startswith("student"):
        # use cached φ schedule estimated from CURRENT_POLICY
