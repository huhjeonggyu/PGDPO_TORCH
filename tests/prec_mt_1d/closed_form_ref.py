# closed_form_ref.py — Dollar closed-form (pre-commitment MV, constant opportunity set)
# ------------------------------------------------------------------------------------
# Outputs DOLLAR control π(t,x) for analytic comparison:
#   π*(t,x) = (Σ^{-1} α) · ( φ(t) - x ),    α = μ - r·1,
#   φ(t)   = ( e^{ρT}/γ + x0 e^{rT} ) e^{-r (T - t)},   ρ = α^T Σ^{-1} α.
#
# This matches Zhou–Li (2000) pre-commitment solution specialized to constant
# opportunity set and converted to dollar control.
#
